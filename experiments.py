...
def get_args():
    ...
    parser.add_argument('--partition', type=str, default='hetero-dir',
    ...
    parser.add_argument('--train_local_student', type=int, default=1,
                        help="whether use PATE to train local student models before aggregation")
    parser.add_argument('--auxiliary_data_portion', type=float, default=0.5,
                        help="the portion of test data that is used as the auxiliary data for PATE")
    ...
    parser.add_argument('--is_local_split', type=int, default=1,
                        help='Whether split the local data for local model training')
    ...
    parser.add_argument('--privacy_analysis_file_name', type=str, default=None,
                        help='The file path to save the information for privacy analysis')
    ...
    parser.add_argument('--local_training_epochs', type=int, default=None,
                        help='the number of epochs for the local trainig alg')
    parser.add_argument('--dp_level', type=int, default=0,
                        help='1 represents add dp on the server side. 2 represents add dp on the party side')
    parser.add_argument('--query_portion', type=float, default=0.5,
                        help='how many queries are used to train the final model')
    parser.add_argument('--local_query_portion', type=float, default=0.5,
                        help='how many queries are used to train the student models')
    ...
    parser.add_argument('--max_z', type=int, default=1,
                        help='the maximum partition that may be influenced when changing a single record')
    ...
    parser.add_argument('--min_require', type=int, default=None,
                        help='require that the minimum number of samples of each class is at least min_require')
    ...
    parser.add_argument('--apply_consistency', type=int, default=1,
                        help='the votes of the party will only be counted if they are the same if set to 1')
    parser.add_argument('--save_global_model', type=int, default=0, help='whether save the global model or not')
    parser.add_argument('--final_stu_epochs', type=int, default=100,
                        help='the number of epochs to train the final student model')
    parser.add_argument('--init_std', type=float, default=-1,
                        help='the stdv for the initialization of the weights, -1 for norm initialization')
    parser.add_argument('--std_place', type=int, default=0, help='1 for std in teacher model, 2 add student model')
    parser.add_argument('--retrain_local_epoch', type=int, default=10,
                        help='the local epoch in fedavg/fedprox after fedkt')
...
def mkdirs(dirpath):
    try:
        '''
        makedirs - creates a leaf directory and all intermediate ones. Works like mkdir,
        except that any intermediate path segment (not just the rightmost) will be created
...
def prepare_uniform_weights(n_classes, net_cnt, fill_val=1):
    ...
    for net_i in range(net_cnt):
        # creates a numpy array with filler values
        temp = np.array([fill_val] * n_classes, dtype=np.float32)
        # creates entry in list - a tensor created from temp numpy array resized to 1 row and inferred column size
...
def normalize_weights(weights):
    ...
    for _, weight in weights.items():
        # if array Z is of size 0, assign it to the first weight item as a numpy array
        if len(Z) == 0:
            Z = weight.data.numpy()
        # once array Z has data in it, keep adding the weight items as numpy arrays
        else:
            Z = Z + weight.data.numpy()

    for mi, weight in weights.items():
        # creates entry in list - the weight divided by a tensor of Z, defined above, plus .000001
...
def train_net_fedprox(net_id, net, global_net, train_dataloader, test_dataloader, epochs, lr, args_optimizer, mu, model,
                      device="cpu"):
    logger.info('Training network %s' % str(net_id))
    logger.info('n_training: %d' % len(train_dataloader))
    logger.info('n_test: %d' % len(test_dataloader))
...
def train_net_scaffold(net_id, net, global_net, train_dataloader, test_dataloader, epochs, lr, args_optimizer, args,
                       server_c, client_c, device="cpu"):
    logger.info('Training network %s' % str(net_id))
    logger.info('n_training: %d' % len(train_dataloader))
    logger.info('n_test: %d' % len(test_dataloader))
...
def local_train_net(nets, args, net_dataidx_map, X_train=None, y_train=None, X_test=None, y_test=None,
                    remain_test_dl=None, local_split=False, retrain_epoch=None, device="cpu"):
    # save local dataset
    # local_datasets = []
    ...
    for net_id, net in nets.items():
        ...
        trainacc, testacc = train_net(net_id, net, train_dl_local, remain_test_dl, n_epoch, args.lr, args.optimizer,
                                      device=device)
...
def local_train_net_fedprox(nets, global_model, args, net_dataidx_map, X_train=None, y_train=None, X_test=None,
                            y_test=None, remain_test_dl=None, local_split=False, retrain_epoch=None, device="cpu"):
    # save local dataset
    # local_datasets = []
    n_teacher_each_partition = args.n_teacher_each_partition
    avg_acc = 0.0
    ...
    for net_id, net in nets.items():
        ...
        if retrain_epoch is not None:
            n_epoch = retrain_epoch
        trainacc, testacc = train_net_fedprox(net_id, net, global_model, train_dl_local, remain_test_dl, n_epoch,
                                              args.lr, args.optimizer, args.mu, args.model, device=device)
        logger.info("net %d final test acc %f" % (net_id, testacc))
        avg_acc += testacc
    avg_acc /= args.n_parties
    if args.alg == 'local_training':
...
def local_train_net_scaffold(nets, global_model, args, net_dataidx_map, X_train=None, y_train=None, X_test=None,
                             y_test=None, server_c=None, clients_c=None, remain_test_dl=None, local_split=False,
                             device="cpu"):
    n_teacher_each_partition = args.n_teacher_each_partition
    avg_acc = 0.0
    if local_split:
    ...
    for net_id, net in nets.items():
        ...
        trainacc, testacc, c_delta = train_net_scaffold(net_id, net, global_model, train_dl_local, remain_test_dl,
                                                        n_epoch,
                                                        args.lr, args.optimizer, args, server_c, clients_c[net_id],
                                                        device=device)
...
def local_train_net_on_a_party(nets, args, net_dataidx_map, party_id, X_train=None, y_train=None, X_test=None,
                               y_test=None, remain_test_dl=None, local_split=0, device="cpu"):
    # save local dataset
    # local_datasets = []
    n_teacher_each_partition = args.n_teacher_each_partition
    if local_split:
    ...
    for net_id, net in nets.items():
        ...
        if remain_test_dl is not None:
            test_dl_global = remain_test_dl
        trainacc, testacc = train_net(net_id, net, train_dl_local, test_dl_global, args.epochs, args.lr, args.optimizer,
                                      device=device)
        # saving the trained models here
        # save_model(net, net_id, args)
        # else:
...
def central_train_net_on_a_party(nets, args, X_train=None, y_train=None, X_test=None, y_test=None, remain_test_dl=None,
                                 local_split=0, device="cpu"):
    # save local dataset
    # local_datasets = []
    n_teacher_each_partition = args.n_teacher_each_partition
...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, no_trans=None):
    if dataset in ('mnist', 'svhn'):
        ...
        train_ds = dl_obj(datadir, dataidxs=dataidxs, split='train', target_type="attr", transform=transform_train,
                          download=True)
...
def get_prediction_labels(models, n_classes, dataloader, args, gamma=None, method="max_vote", train_cls_counts=None,
                          uniform_weights=True, sanity_weights=False, is_subset=0, is_final_student=False,
                          device="cpu"):
    # correct, total = 0, 0
    # true_labels_list = []
    pred_labels_list = []
    ...
    with torch.no_grad():
        for batch_idx, (x, target, index) in enumerate(dataloader):
            ...
            if method == "averaging":
                ...
                vote_counts_save = np.append(vote_counts_save,
                                             vote_counts_real.view(-1, x.data.size()[0]).to("cpu").numpy(), axis=1)
...
def train_a_student(tea_nets, public_dataloader, public_ds, remain_test_dataloader, stu_net, n_classes, args,
    ...
    if args.pub_datadir is not None:
        is_subset = 0
    public_labels, top2_counts_differ_one, vote_counts_save = get_prediction_labels(tea_nets, n_classes,
                                                                                    public_dataloader, args,
                                                                                    gamma=gamma,
                                                                                    method=args.ensemble_method,
                                                                                    is_subset=is_subset,
                                                                                    is_final_student=is_final_student,
                                                                                    device=device)
    if filter_query:
        confident_query_idx = []
        ...
        public_dataloader = data.DataLoader(dataset=public_ds, batch_size=32,
                                            sampler=data.SubsetRandomSampler(confident_query_idx),
                                            num_workers=n_workers)
    ...
    if args.optimizer == 'adam':
        optimizer = optim.Adam(filter(lambda p: p.requires_grad, stu_net.parameters()), lr=args.stu_lr,
        ...
        optimizer = optim.SGD(filter(lambda p: p.requires_grad, stu_net.parameters()), lr=args.stu_lr, momentum=0.9,
                              weight_decay=args.reg)
    criterion = nn.CrossEntropyLoss().to(device)
    ...
    for n_exp in range(args.trials):
        ...
        if args.model == 'lr':
            ...
            if args.dataset in libsvm_datasets:
                ...
                public_ds = CustomTensorDataset(torch.tensor(X_test[:public_data_size].toarray(), dtype=torch.float32),
                                                torch.tensor(y_test[:public_data_size], dtype=torch.long))
                remain_test_ds = CustomTensorDataset(
                    torch.tensor(X_test[public_data_size:].toarray(), dtype=torch.float32),
                    torch.tensor(y_test[public_data_size:], dtype=torch.long))
            ...
            if args.dataset != 'xray':
                ...
                public_ds = PneumoniaDataset(root=args.pub_datadir + './stage_2_train_images/', subset='train',
                                             pIds=pIds_train, transform=transform, warping=True)
                ...
                if query_data_size == public_data_size and local_query_data_size == public_data_size:
                    ...
                    local_query_dl = data.DataLoader(dataset=public_ds, batch_size=32,
                                                     sampler=data.SubsetRandomSampler(
                                                         list(range(local_query_data_size))), num_workers=n_workers)
        ...
        if args.alg == 'fedavg':
            ...
            for round in range(args.comm_round):
                ...
                local_train_net_fedprox(nets, global_model, args, net_dataidx_map, X_train, y_train, X_test, y_test,
                                        remain_test_dl=remain_test_dl, local_split=False, device=device)
                global_model.to('cpu')
            ...
            for round in range(args.comm_round):
                ...
                local_train_net_scaffold(nets, global_model, args, net_dataidx_map, X_train, y_train, X_test, y_test,
                                         server_c, clients_c, remain_test_dl=remain_test_dl, local_split=False,
                                         device=device)
                global_model.to('cpu')
            ...
            if args.model == 'tree':
                ...
                public_data_size = int(len(y_test) * args.auxiliary_data_portion)
                param = {'max_depth': args.max_tree_depth, 'objective': 'binary:logistic', 'gamma': 1, 'lambda': 1,
                         'eta': 0.1}
                for party_id in range(args.n_parties):
                    gbdt = xgb.XGBClassifier(max_depth=args.max_tree_depth, n_estimators=args.n_stu_trees,
                                             learning_rate=args.lr, gamma=1, reg_lambda=1, tree_method='hist')
                ...
                local_train_net(nets, args, net_dataidx_map, X_train, y_train, X_test, y_test,
                                remain_test_dl=remain_test_dl, local_split=False,
                                device=device)
            ...
            nets_list = local_train_net(nets, args, net_dataidx_map, X_train, y_train, X_test, y_test,
                                        local_split=False, device=device)
            train_dl_global, test_dl_global, _, _ = get_dataloader(args.dataset, args.datadir, args.batch_size, 32)
            logger.info("Compute uniform ensemble accuracy")
            uens_train_acc, _ = compute_ensemble_accuracy(nets_list, train_dl_global, n_classes,
                                                          ensemble_method=args.ensemble_method, uniform_weights=True,
                                                          device=device)
            uens_test_acc, _ = compute_ensemble_accuracy(nets_list, test_dl_global, n_classes,
                                                         ensemble_method=args.ensemble_method, uniform_weights=True,
                                                         device=device)
            ...
            if args.model != 'tree':
                ...
                exit(1)
            logger.info("Initializing trees")
            trees = init_trees(args.max_tree_depth, args.n_parties, args.n_teacher_each_partition,
                               libsvm_datasets[args.dataset])
            ...
            if args.model == 'tree' or args.model == 'gbdt' or args.model == 'gbdt_tree':
                ...
                trees = init_trees(args.max_tree_depth, 1, args.n_teacher_each_partition, libsvm_datasets[args.dataset],
                                   args)
                ...
                if args.model == 'tree':
                    ...
                    stu_forest = xgb.XGBClassifier(max_depth=args.max_tree_depth, n_estimators=args.n_stu_trees,
                                                   learning_rate=args.lr,
                                                   gamma=1, reg_lambda=1, tree_method='hist')
                filter_query = args.filter_query
                if args.dp_level == 2:
                    gamma = args.gamma
                    top2_counts_differ_one, vote_counts_in_a_party = train_a_student_tree(trees, X_test[
                                                                                                 :local_query_data_size],
                                                                                          y_test[
                                                                                          :local_query_data_size], 2,
                                                                                          stu_forest, gamma,
                                                                                          filter_query)
                else:
                    gamma = 0
            ...
            if args.model == 'tree' or args.model == 'gbdt':
                ...
                for party_id in range(args.n_parties):
                    ...
                    local_train_trees_in_a_party(trees, args, net_dataidx_map[party_id], X_train, y_train, X_test,
                                                 y_test)
                ...
                if args.dp_level == 2:
                    gamma = args.gamma
                    top2_counts_differ_one, vote_counts_in_a_party = train_a_student_tree(tree_list, X_test[
                                                                                                     :local_query_data_size],
                                                                                          y_test[
                                                                                          :local_query_data_size], 2,
                                                                                          stu_forest, gamma,
                ...
                for party_id in range(args.n_parties):
                    nets, _, _ = init_nets(args.net_config, args.dropout_p, args.n_parties, args,
                                           args.n_teacher_each_partition)
                    nets_list = local_train_net_on_a_party(nets, args, net_dataidx_map, party_id, X_train, y_train,
                                                           X_test,
            ...
            if args.model == 'tree' or args.model == 'gbdt' or args.model == 'random_forest' or args.model == 'gbdt_ntree' or args.model == 'gbdt_tree':
                ...
                for party_id in range(args.n_parties):
                    ...
                    for i in range(args.n_partition):
                        trees = init_trees(args.max_tree_depth, 1, args.n_teacher_each_partition,
                                           libsvm_datasets[args.dataset], args)
                        logger.info("In party %d Train local trees" % party_id)
                        local_train_trees_in_a_party(trees, args, net_dataidx_map[party_id], X_train, y_train, X_test,
                                                     y_test)
                        if args.train_local_student:
                            if args.model == 'tree' or args.model == 'random_forest':
                                stu_forest = RandomForestClassifier(max_depth=args.max_tree_depth,
                                                                    n_estimators=args.n_stu_trees)
                            elif args.model == 'gbdt' or args.model == 'gbdt_ntree' or args.model == 'gbdt_tree':
                                stu_forest = xgb.XGBClassifier(max_depth=args.max_tree_depth,
                                                               n_estimators=args.n_stu_trees, learning_rate=args.lr,
                                                               gamma=1, reg_lambda=1, tree_method='hist')
                            filter_query = args.filter_query
                            if i < args.max_z:
                                filter_query = 0
                ...
                if args.alg == 'simenb':
                    simple_ensemble_acc = compute_tree_ensemble_accuracy(tea_trees, X_test[public_data_size:],
                                                                         y_test[public_data_size:])
                    logger.info("simple ensemble acc: %f" % simple_ensemble_acc)
                    exit(0)
                # vote_counts_parties = np.reshape(vote_counts_parties, (args.n_parties,-1))
                if args.model == 'tree' or args.model == 'random_forest':
                    final_forest = RandomForestClassifier(max_depth=args.max_tree_depth,
                                                          n_estimators=args.n_final_stu_trees)
                elif args.model == 'gbdt' or args.model == 'gbdt_ntree' or args.model == 'gbdt_tree':
                    final_forest = xgb.XGBClassifier(max_depth=args.max_tree_depth, n_estimators=args.n_final_stu_trees,
                                                     learning_rate=args.lr,
                                                     gamma=1, reg_lambda=1, tree_method='hist')
                if args.dp_level == 1:
                    gamma = args.gamma
                    _, vote_counts = train_a_student_tree(tea_trees, X_test[:query_data_size],
                                                          ...
                                                          args.prob_threshold,
                                                          args.n_partition, args.apply_consistency,
                                                          is_final_student=True)
                else:
                    gamma = 0
                    _, vote_counts = train_a_student_tree(tea_trees, X_test[:query_data_size],
                                                          y_test[:query_data_size], 2, final_forest, gamma, 0,
                ...
                if args.train_local_student:
                    np.savez(file_path2, n_instances_portion, n_parti_top2_differ_one,
                             vote_counts_parties.reshape(-1, 2))
                ...
                for party_id in range(args.n_parties):
                    ...
                    for i in range(args.n_partition):
                        ...
                        nets_list = local_train_net_on_a_party(nets, args, net_dataidx_map, party_id, X_train, y_train,
                                                               X_test, y_test, remain_test_dl,
                                                               local_split=args.is_local_split,
                                                               device=device)
                        # nets_list_partition.append(nets_list)
                        ...
                        if args.train_local_student:
                            ...
                            local_stu_test_acc, conf_mat = compute_accuracy(stu_net, remain_test_dl,
                ...
                if args.alg == 'simenb':
                    simple_ensemble_acc, _ = compute_ensemble_accuracy(tea_nets, remain_test_dl, n_classes,
                                                                       ensemble_method=args.ensemble_method,
                                                                       uniform_weights=True, device=device)
                    logger.info("simple ensemble acc:%f" % simple_ensemble_acc)
                    exit(0)
                if args.with_unlabeled:
                    ...
                    if args.dp_level == 1:
                        gamma = args.gamma
                        _, _, vote_counts = train_a_student(tea_nets, query_dl, public_ds, remain_test_dl,
                                                            global_stu_net,
                                                            n_classes, args, gamma=args.gamma,
                                                            is_subset=1, is_final_student=True, filter_query=0,
                                                            device=device)
                    else:
                        gamma = 0
                        _, _, vote_counts = train_a_student(tea_nets, query_dl, public_ds, remain_test_dl,
                    ...
                    global_stu_test_acc, conf_mat = compute_accuracy(global_stu_net, remain_test_dl,
                                                                     get_confusion_matrix=True, device=device)
                ...
                if args.train_local_student:
                    np.savez(file_path2, n_instances_portion, n_parti_top2_differ_one,
                             vote_counts_parties.reshape(-1, 10))
